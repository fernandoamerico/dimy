...
import { 
  ...
  List, MousePointerClick, LayoutTemplate 
} from 'lucide-react';
...
export function CategoryBuilder({
  ...
  const handleSaveCategory = async () => {
    ...
    if (ok) {
      toast.success('Configurações da Categoria salvas!');
      router.refresh();
    } else {
  ...
  const handleUpdateFieldName = async (index: number, rawNewName: string) => {
    ...
    if (!newName.trim()) { toast.error('A chave da API não pode ser vazia.'); return; }
    if (fields[index].name === newName) return; 
  ...
  const renderFieldEditor = (field: any) => {
    ...
    switch (field.type) {
      case 'text':
        return <div className={`${inputClass} h-12`}>Campo de texto curto será preenchido na publicação.</div>;
      case 'richText':
        return <div className={`${inputClass} h-24`}>Área de texto longo será preenchida na publicação.</div>;
      case 'wysiwyg':
        return <div className={`${inputClass} h-32 flex-col`}><LayoutTemplate className="w-5 h-5 mb-2 opacity-50"/>Editor visual (WYSIWYG) será preenchido na publicação.</div>;
      case 'image':
        return <div className={`${inputClass} h-24 flex-col`}><ImageIcon className="w-5 h-5 mb-2 opacity-50"/> Imagem será preenchida na publicação.</div>;
      case 'gallery':
        return <div className={`${inputClass} h-24 flex-col`}><ImageIcon className="w-5 h-5 mb-2 opacity-50"/> Galeria de Imagens será preenchida na publicação.</div>;
      case 'url':
        return <div className={`${inputClass} h-12`}>URL de destino será preenchida na publicação.</div>;
      case 'table':
        return <div className={`${inputClass} h-24 flex-col`}><List className="w-5 h-5 mb-2 opacity-50"/> Tabela será preenchida na publicação.</div>;
      case 'button':
        return <div className={`${inputClass} h-16 flex-col`}><MousePointerClick className="w-5 h-5 mb-1 opacity-50"/>Botão será preenchido na publicação.</div>;
      default:
        return null;
    }
  };
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Configure a estrutura base para as publicações.</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          {fields.length === 0 ? (
            <div className="bg-white/60 dark:bg-neutral-900/60 backdrop-blur-md border border-dashed border-gray-300 dark:border-neutral-700 rounded-2xl p-12 text-center flex flex-col items-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-sm">Adicione blocos ao lado para definir o esqueleto que todos os posts desta categoria terão.</p>
            </div>
          ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl p-5 shadow-sm border border-slate-200 dark:border-neutral-800">
            ...
            <p className="text-[10px] text-gray-400 dark:text-neutral-500 mt-4">Estas opções habilitam/desabilitam funcionalidades no painel lateral de criação de publicações para esta categoria.</p>
          </div>
