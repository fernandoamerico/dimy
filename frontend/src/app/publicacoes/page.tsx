        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Gerencie suas postagens em diferentes categorias (ex: Blog, Notícias, Tutoriais).
            </p>
          </div>
        ...
        {categories.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center p-12 bg-white/50 dark:bg-neutral-900/50 rounded-3xl border border-dashed border-gray-200 dark:border-neutral-800">
            ...
            <p className="text-gray-500 dark:text-gray-400 max-w-md mb-8">
              Você ainda não possui nenhuma categoria de publicação. Crie sua primeira categoria, como "Blog" ou "Notícias", para começar a postar.
            </p>
            <button
              onClick={() => setIsModalOpen(true)}
            ...
            {categories.map((cat) => {
              ...
              return (
                <div key={cat.id} className="group flex flex-col bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 dark:shadow-sm dark:hover:shadow-md hover:border-blue-200 dark:hover:border-emerald-500/30 transition-all overflow-hidden relative">
                  <Link href={`/publicacoes/list?slug=${cat.slug}`} className="p-5 flex-1 block">
                  ...
                  return (
                    <tr key={cat.id} className="hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition-colors group">
                      <td className="px-6 py-4">
                        <Link href={`/publicacoes/list?slug=${cat.slug}`} className="flex items-center gap-3 group-hover:text-blue-600 dark:group-hover:text-emerald-400 transition-colors">
                          <Folder className="w-5 h-5 text-gray-400 group-hover:text-blue-600 dark:group-hover:text-emerald-400" />
                          <div>
